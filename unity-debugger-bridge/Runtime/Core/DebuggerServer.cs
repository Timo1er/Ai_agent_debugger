...

            {
                ...
                if (method == "oracle.microAudit")
                {
                    ...
                    return;
                                ...
                                // === GAMEPLAY DEEP LOGIC AUDIT ===
                if (method == "gameplay.auditBugs")
                {
                    var bugsList = DeepGameplayLogicAuditor.Instance != null ? DeepGameplayLogicAuditor.Instance.AuditAllGameplayScripts() : new List<object>();
                    client.SendResponse(request.id, bugsList);
                    return;
                }
